        ...
        $i = 1;
            while ($reg = $query_Pedido->fetch_object()) {
                ...
                $data[] = array("0"=>$i,
                    ...
                    "9"=>($reg->estado=="A")?'<button class="btn btn-success" data-toggle="tooltip" title="Ver Detalle" onclick="cargarDataPedido('.$reg->idpedido.',\''.$reg->tipo_pedido.'\',\''.$reg->numero.'\',\''.$reg->Cliente.'\',\''.$reg_total->Total.'\',\''.$reg->email.'\')" ><i class="fa fa-eye"></i> </button>&nbsp'.
    ...
    case "GetDetallePedido":
        ...
        $i = 1;
            while ($reg = $query_prov->fetch_object()) {
                 echo '<tr>
                        <td>'.$reg->articulo.' - '.$reg->nombreGeneral.'</td>
                        <td>'.$reg->codigo.'</td>
                        <td>'.$reg->serie.'</td>
...
/*
    ...
    case "listDetIng":
        require_once "../model/Pedido.php";
            // $objArticulo = new Pedido();
            // $query_Tipo = $objArticulo->ListarDetalleIngresos($_SESSION["idsucursal"]);
            // $data = Array();
            // $i = 1;
            // while ($reg = $query_Tipo->fetch_object()) {
            //     $data[]=array($reg->Articulo);
            // echo $reg->Articulo."----------".$_SESSION["idsucursal"]."-------";
        ...
        $objPedido = new Pedido();
        $query_clis = $objPedido->ListarDetalleIngresos($_SESSION["idsucursal"]);

        $data= Array();
        $i = 1;
            while ($reg = $query_clis->fetch_object()) {
                $data[] = array(

                    "0"=>'<button type="button" class="btn btn-warning" name="optDetIngBusqueda[]" data-codigo="'.$reg->nlote.'" 
                    data-serie="'.$reg->regsanitario.'" data-nombre="'.$reg->Articulo.'" data-precio-venta="'.$reg->precio_ventapublico.'" 
                    ...
                    onclick="AgregarPedCarrito('.$reg->iddetalle_ingreso.',\''.$reg->stock_unidad.'\',\''.$reg->Articulo.'\',\''.$reg->nombreGeneral.'\',\''.$reg->nlote.'\',\''.$reg->regsanitario.'\',\''.$reg->precio_ventapublico.'\')" >
