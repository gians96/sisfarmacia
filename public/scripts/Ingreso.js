...
(function( $ ) {
 ...
 function init() {
    ...
    $.post("./ajax/IngresoAjax.php?op=listCb_Laboratorio", function(r){ vcblaboratorio=r; return r});
...
function Agregar(){
    ...
    var prec_ventaP = document.getElementsByName("txtPrecioVentaP");
        ...
        var c_laboratorio = document.getElementsByName("cbolaboratorio");
        ...
        if ($("#cboTipoComprobanteIng").val() != "") {
            if (elementos.length > 0) {
                ...
                var data = { 
                    ...
                    total : $("#txtTotal").val(),
                    fecha_emision: $("#cboFechaEmision").val(),
                    detalle : detalle
                };
        ...
        if ($("#txtIdIngreso").val() != "") {//Cuando se edita la factura los productos como cantidad precio
            ...
            for (var pos in data) {
                if(data[pos][4]=="1"){//Es cuando los productos vienen por unidad
                    ...
                    "<td><div hidden=''><select   id='cbolaboratorio[]' name='cbolaboratorio' class='form-control'></select></div>"+data[pos][14] +"</td>"+
                ...
                }else{//cuando esta tienen mas de >1
             ...
             $('table#tblDetalleIngreso th:nth-child(12)').hide();
            $('table#tblDetalleIngreso th:nth-child(11)').hide();
            // style='display:none;'
            for (var pos in data) {
            ...
            "<td style='display:none;'  WIDTH='20'><input class='form-control' type='text' onkeypress='return justNumbers(event);' name='txtStockAct' id='txtStockAct["+pos+"]'   value='" + data[pos][10] + "'  onkeyup='calcularStockUnidad(" + pos + ");calcularTotal(" + pos + ");' onkeypress='calcularStockUnidad(" + pos + ");' required /></td>"+
            "<td style='display:none;'><input class='form-control' type='text'  onkeypress='return justNumbers(event);'  name='txtStockUnidadM' onkeyup='Modificar(" + pos + ");calcularTotal(" + pos + ");' id='txtStockUnidadM["+pos+"]' value='0' />"+
            ...
            "<td><select id='cbolaboratorio["+pos+"]' onclick='Modificar(" + pos + ");' name='cbolaboratorio' class='form-control'>"+vcblaboratorio+"</select></td>"+
...
function ListadoIngresos(){ 
    ...
    function cancelarIngreso(idIngreso){
        bootbox.confirm("¢ÄEsta Seguro de Anular el ingreso?", function(result){
            if(result){
...
function copyIngreso_actual(posi){

 var data = JSON.parse(objinit.consultar());

 var CajaIngreso = document.getElementsByName("txtCajaIngreso");
 var UniIngreso = document.getElementsByName("txtUniIngreso");
 var StockIng = document.getElementsByName("txtStockIng");
 
    for (var pos in data) {



        $("[name=txtStockAct]:eq("+pos+")").val(CajaIngreso[pos].value); 
        $("[name=txtStockUnidadM]:eq("+pos+")").val(UniIngreso[pos].value); 
        $("[name=txtStockUnidad]:eq("+pos+")").val(StockIng[pos].value);  
...
//         }
    ...
    function Modificar(pos){
     ...
     var vlaboratorio=document.getElementsByName("cbolaboratorio");
        ...
        elementos[pos][14] = vlaboratorio[pos].value;
    ...
    function AgregarDetalle(idart, nombre, codigo, serie, descripcion, stock_ing, stock_act, p_compra, p_ventaD, p_ventaP, pos) {
        var cant = prompt("¢ÄCuantas series necesita?", "");
    ...
    function Agregar(id, art,lab,unidad){
        ...
        for (var pos in data) {
            ...
            $("select[id='cbolaboratorio["+pos+"]']").find("option[value='"+elementos[pos][14]+"']").attr("selected",true);
