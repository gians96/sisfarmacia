...
function ListadoProveedor(){ 
        // alert('entra proveedor');

        var tabla = $('#tblProveedorLaboratorio').dataTable(
        {   "aProcessing": true,
            "aServerSide": true,
            dom: 'Bfrtip',
            buttons: [
            ...
            "aoColumns":[
            ...
            ],"ajax": 
                {
                    ...
                    error: function(XMLHttpRequest, textStatus, errorThrown){
                        console.log("error"+XMLHttpRequest.responseText+"-"+textStatus+"-"+errorThrown);    
                    }
                },
            "bDestroy": true

        }).DataTable();
        // alert('entra proveedor'+tabla);
    };
...
function cargarDataProveedor(id, tipo_persona,nombre,tipo_documento,num_documento,direccion_departamento,direccion_provincia,direccion_distrito,direccion_calle,telefono,email,numero_cuenta,estado){// funcion que llamamos del archivo ajax/CategoriaAjax.php linea 52
		...
		// alert('tipo perspma'+tipo_persona);
 	// 	$("#cboTipo_Persona").val(tipo_persona);
 		$("#cboTipo_Persona option[value="+ tipo_persona +"]").attr("selected",true);
