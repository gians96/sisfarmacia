...
function EnviarCorreo(){
                    ...
                    swal("Mensaje del Sistema", r, "success");
                            ...
                            $.getJSON("./ajax/PedidoAjax.php?op=GetIdPedido", function(r) {
                                if (r) {
                                    ...
                                    $('table#tblDetallePedidoVer th:nth-child(9)').hide();

                                    $('table#tblDetallePedido th:nth-child(9)').hide();
    ...
    function ComboTipoDoc() {
        ...
        $.get("./ajax/PedidoAjax.php?op=listTipoDoc", function(r) {
                ...
                $("#cboTipoComprobante").html( "<option>--Seleccione Comprobante--</option><option value='TICKET'>TICKET</option>");
...
function ConsultarDetallesPed() {
    ...
    for (var pos in data) {
        ...
        $("table#tblDetallePedido").append("<tr>"+
            "<td>" + data[pos][1] + " <input class='form-control' type='hidden' name='txtIdDetIng' id='txtIdDetIng[]' value='" + data[pos][0] + "' /></td>"+
            "<td> " + data[pos][6] + "</td>"+
            "<td> " + data[pos][7] + "</td>"+
            "<td>" + data[pos][5]+ "</td>"+
            "<td><input class='form-control' type='text' name='txtPrecioVentPed' id='txtPrecioVentPed[]' value='" + data[pos][2] + "' onkeyup='calcularTotalPed(" + pos + ")' /></td>"+
            "<td><input class='form-control' type='text' name='txtCantidaPed' id='txtCantidaPed[]'  value='" + data[pos][3] + "' onkeyup='calcularTotalPed(" + pos + ")' /></td>"+
            "<td><input class='form-control' type='text' name='txtDescuentoPed' id='txtDescuentoPed[]' value='" + data[pos][4] + "' onkeyup='calcularTotalPed(" + pos + ")' /></td>"+
            "<td><input class='form-control' type='text' name='txtSubtotal' id='txtSubtotal["+pos+"]' value='0' onkeyup='calcularTotalPed(" + pos + ")' /></td>"+
            "<td><button type='button' onclick='eliminarDetallePed(" + pos + ")' class='btn btn-danger'><i class='fa fa-remove' ></i> </button></td></tr>");
    }
    calcularIgvPed();
    calcularSubTotalPed();
    calcularTotalPed();
...
function calcularSubTotalPed(){
    ...
    for (var pos in data) {
        suma += parseFloat((data[pos][3] *  data[pos][2]) - data[pos][4]);
        $("input[id='txtSubtotal["+pos+"]']").val(parseFloat((data[pos][3] *  data[pos][2]) - data[pos][4]));
...
function cancelarPedido(idPedido){
       // alert(idPedido);
            ...
            bootbox.confirm("è¢ƒEsta Seguro de Anular la Venta?", function(result){
        ...
        function eliminarPedido(idPedido){
            bootbox.confirm("è¢ƒEsta Seguro de eliminar el pedido?", function(result){
                if(result){
    ...
    function ComboTipoDoc() {
        ...
        $.get("./ajax/PedidoAjax.php?op=listTipoDoc", function(r) {
                // $("#cboTipoComprobante").html(r);
                $("#cboTipoComprobante").html( "<option value='TICKET'>TICKET</option>");

                
            })
    }

    function AgregarPedCarrito(iddet_ing, stock_actual, art,des, cod, serie, precio_venta){
     
        if (stock_actual > 0) {
            var detalles = new Array(iddet_ing, art+" - "+des, precio_venta, "1", "0.0", stock_actual, cod, serie);
            elementos.push(detalles);
            ConsultarDetallesPed();
            $('table#tblDetallePedido th:nth-child(4)').show();
            $('table#tblDetallePedido th:nth-child(9)').show();
