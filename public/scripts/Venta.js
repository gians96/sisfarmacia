...
function init(){
    ...
    $("#btnrefreshPedido").click(RefreshTablePedido);
    $("form#frmCreditos").submit(SaveCredito);
    $("#btnCancelarPedido").click(OcultarFormPedido);
    $("#btnCancelarGenerarVenta").click(OcultarFormGenerarVenta);

    function ComboTipo_Documento() {

        $.get("./ajax/PedidoAjax.php?op=listTipoDoc", function(r) {
                // $("#cboTipoComprobante").html(r);
                $("#cboTipoComprobante").html( "<option value='0'>--Seleccione Comprobante--</option><option value='TICKET'>TICKET</option>");
    ...
    function VerFormPedido(){
        ...
        $("#btnReporte").hide();
        $("#btnrefreshPedido").hide();//ocultamos el boton refrescar la tabla pedidos
        $("#VerTotalesDetPedido").show();// Mostramos el formulario del igv sub total E igv
        $('button[type="submit"]').show();//submit pedido

           $("#btnBuscarCliente").prop("disabled", false);
        $("#btnBuscarDetIng").prop("disabled", false);
    }

    function OcultarFormPedido(){
        $("#VerFormPed").hide();// Mostramos el formulario
        $("#btnNuevoPedido").show();// ocultamos el boton nuevo
        $("#btnGenerarVenta").show();
        $("#VerListado").show();// ocultamos el listado
        $("#btnReporte").show();
        $("#btnrefreshPedido").show();
    ...
    function OcultarFormGenerarVenta(){
...
function eliminarVenta(id){// funcion que llamamos del archivo ajax/CategoriaAjax.php?op=delete linea 53
	bootbox.confirm("è¢ƒEsta Seguro de eliminar el Venta seleccionado?", function(result){ // confirmamos con una pregunta si queremos eliminar
...
function pasarIdPedido(idPedido, total, correo){// funcion que llamamos del archivo ajax/CategoriaAjax.php linea 52
		//////////////////////////
            $("#frmVentas").show();// Mostramos el formulario
        $("#btnNuevoPedido").hide();// ocultamos el boton nuevo
        $("#btnGenerarVenta").hide();
        $("#VerListado").hide();// ocultamos el listado
        $("#btnReporte").hide();
        ...
        $("#cboTipoComprobante").val("0");
        // $("select[id='cboTipoComprobante']").find("option[value='0']").attr("selected",true);
