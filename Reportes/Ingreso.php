...
// Client address
function addLaboratorioAdresse( $cliente,$domicilio,$num_documento,$email,$telefono )
{
	$r1     = $this->w - 100;
	$r2     = $r1 + 68;
	$y1     = 40;
	$this->SetXY( $r1, $y1);
	$this->SetFont( "Arial", "B", 10);
	$this->MultiCell( 60, 4, "LABORATORIO");
	$this->SetXY( $r1, $y1+5);
	$this->SetFont( "Arial", "", 10);
	$this->MultiCell( 150, 4, $cliente);
	$this->SetXY( $r1, $y1+10);
...
function addTVAs( $igv, $total,$moneda )
{
	...
	$this->SetFont( "Arial", "", 8);
	$this->SetXY( $re, $y1+5 );
	$this->Cell( 17,4, $moneda.sprintf("%0.2F", $total-($total*$igv/(100+$igv))), '', '', 'R');
	$this->SetXY( $re, $y1+10 );
	$this->Cell( 17,4, $moneda.sprintf("%0.2F", ($total*$igv/(100+$igv))), '', '', 'R');
